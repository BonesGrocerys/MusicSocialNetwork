...
public class PlaylistMapping : Profile
{
    public PlaylistMapping()
    {
        ...
        CreateMap<PlaylistUpdateNameRequest, Playlist>().ForMember(x => x.PlaylistImage, opt => opt.Ignore());
        CreateMap<PlaylistUpdateImageRequest, Playlist>().ForMember(x => x.PlaylistImage, opt => opt.Ignore());
    }
}
